...
import { shareWithUser } from 'services/axios/brick';
...
const ShareWithUserDialog: React.FC<InviteProps> = ({ brick, ...props }) => {
  ...
  const inviteUserById = async (userId: number) => {
    let success = await shareWithUser(brick.id, userId);
    if (success) {
      props.submit(getNameText(), accessGranted || false);
    } else {
      // failed
    ...
    >
      ...
      <div className="dialog-header" style={{ minWidth: '30vw' }}>
        <div className="title left">
          {props.title ? props.title : 'Who would you like to invite to see this brick?'}
        </div>
...
export default connector(ShareWithUserDialog);
