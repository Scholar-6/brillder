...
const QuillToolbarColorSelect: React.FC<QuillToolbarColorSelectProps> = props => {
  ...
  return (
    ...
    >
      ...
      <span id={`picker-options-${uniqueId}`} className="picker-options" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); }} onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}>
        {props.children && React.Children.map(props.children as any, getItem)}
      </span>
      {props.enabled && <div className="css-custom-tooltip tooltip-colors">Change Colours</div>}
    </span>
  );
};
