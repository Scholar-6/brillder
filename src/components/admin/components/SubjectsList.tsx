...
class SubjectsList extends Component<PublishedSubjectsProps, any> {
  render() {
    return (
      <Grid container direction="row" className="filter-container subjects-filter admin-subject-filter">
        <AnimateHeight
          duration={500}
        ...
        >
          {this.props.subjects.map((subject, i) => (
            <Grid key={i} container direction="row" className={`subject-list-v2 ${subject.checked ? 'checked' : ''}`} onClick={() => this.props.filterBySubject(i)}>
              <Grid item xs={12} className="filter-container subjects-indexes-box">
                <FormControlLabel
