...
export const prepareVisibleAssignments = (sortedIndex: number, pageSize: number, assignments: AssignmentBrick[]) => {
  ...
  for (let i = 0 + sortedIndex; i < pageSize + sortedIndex; i++) {
    ...
    if (assignment) {
      ...
      data.push({
        brick: assignment.brick, key: i, index: count, assignmentId: assignment.id, status: assignment.status, row, deadline: assignment.deadline, completedDate: assignment.completedDate, teacher: assignment.teacher
